...
export default function PlatformPagesAdminPage() {
  ...
  function handleDelete(page: PlatformPage) {
    confirm({
      message: `Delete "${page.title}"? This removes it from the platform's own public pages.`,
      confirmLabel: "Delete",
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <p className="text-sm text-zinc-500 dark:text-zinc-400">
        Your own platform&rsquo;s content (About Us, Refund &amp; Cancellation Policy, ...) — global,
        not tied to any tenant. Shown at <code>/platform/[slug]</code>.
      </p>
      {error && (
              ...
              editingId === page.id ? (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <Toggle checked={page.isPublished} onChange={() => handleTogglePublished(page)} />
                    <button
                      type="button"
...
function PageForm({
  ...
  initial: PlatformPageInput;
  onCancel: () => void;
  onSave: (input: PlatformPageInput) => Promise<void>;
}) {
  const { showToast } = useToast();
