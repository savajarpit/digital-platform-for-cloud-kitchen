...
/**
 * The image/name/description area links to the meal's own detail page
 * (gallery, full description, customize section); the price+add row below
 * it is a sibling, not nested inside that link, so the quick-add button
 * never fights the card's own navigation click.
 */
  ...
  return (
    ...
    >
      ...
      <div className="mt-auto flex items-center justify-between gap-2 p-4 pt-3">
        ...
        <AddToCartButton meal={meal} priceInPaise={discountedPriceInPaise} disabled={!meal.isAvailable} />
      </div>
    </article>
  );
}
